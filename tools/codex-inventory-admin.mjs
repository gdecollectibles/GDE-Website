...
const labelsFor = (products, getter) => [...new Map(Object.values(products).map(p => [getter(p), getter(p)])).values()];
...
const productCard = ([id, p]) => {
  ...
  return `<article class="product-card" data-type="${slug(p.type)}" data-product="${id}" data-platform="${platformSlug(p.platform)}" data-finish="${slug(p.finish)}" data-price="${priceNumber(p.price)}" data-availability="${availability}"><a class="product-image ${p.image}" href="product.html?product=${id}"${imageStyle}><span class="status ${availability}">${escapeHtml(p.availability)}</span></a><div class="product-info"><p class="meta">${escapeHtml(p.type)} &middot; ${escapeHtml(p.platform)} &middot; ${escapeHtml(p.finish)}</p><h2>${escapeHtml(p.name)}</h2><p class="price">${escapeHtml(p.price)}</p><div class="card-actions">${archived ? `<a class="button full disabled" aria-disabled="true">Contact Us</a>` : `<a class="button full add-to-cart-link" href="checkout.html?product=${id}" data-product="${id}">ADD TO CART</a>`}<a class="text-link" href="product.html?product=${id}">View Details &rarr;</a></div></div></article>`;
};
...
function generate() {
  ...
  const navLinks = entries
    ...
    .join("\n");
  script = script.replace(/        <p>Individual collections<\/p>\r?\n[\s\S]*?      <\/div>`;/, `        <p>Individual collections</p>\n${navLinks}\n      </div>\`;`);
  ...
  const productFilters = checkboxGroup("Collections", "product", entries.map(([, p]) => p.name), value => entries.find(([, p]) => p.name === value)?.[0] || slug(value));
  ...
  collections = collections.replace(/        <div class="filter-group"><h3>Collections<\/h3>[\s\S]*?        <button class="button secondary full reset-filters"/, `        ${filters}\n        <button class="button secondary full reset-filters"`);
  collections = collections.replace(/<p id="resultsLabel">Showing all \d+ collections<\/p>/, `<p id="resultsLabel">Showing all ${entries.length} collections</p>`);
